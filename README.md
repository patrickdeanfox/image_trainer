...
NSFW-focused single-column layout. Top bar: **Generate** · **Stop** (shown only while a run is active) · **Compare stacks** · **Compare LoRAs** · **Load run_info…** · **Open outputs** · progress bar.

- **Live auto-persist.** Every field (prompt body, negative, sampler, steps, guidance, seed, aspect, LoRA picks + weights, quality stack, base override) saves to `<projects_root>/.user_settings.json` with a 400 ms debounce as you type or click. Atomic writes — temp file plus `os.replace` — so a crash mid-write can't leave a corrupt JSON behind. No save buttons; your last state restores on next launch automatically.
- **Stop button (graceful kill).** While any generate / compare run is in flight, clicking Stop sends SIGINT to the subprocess. The diffusers pipeline observes the signal at the next denoising-step boundary, finishes the current image cleanly, writes whatever's done to disk, and exits. Hidden when no run is active.
- **Live prompt builder.** Subject / Scene / Action dropdowns rewrite the prompt body the moment any pick changes. The "Build prompt from picks ▶" button is a manual rebuild for after hand-editing. Subject group: count (solo / couple / threesome / group), gender (woman / man / trans woman / trans man / futanari / non-binary), hair / eyes / skin / body / breast / makeup. Action group: generic activity (posing / walking / showering / etc.), **dedicated Sex-act dropdown** (32 options across solo, oral, vaginal, anal, group, and outcome variants), outfit, pose, expression. The Sex-act axis is independent of Activity so SFW workflows aren't polluted with explicit options.
...
- **Recommendations sidebar** (Best NSFW base checkpoints + Civitai LoRAs + Photoreal survival guide) is collapsed by default — each entry shows just name + ⓘ icon; hover for the hint. Saves ~250 px of always-visible text vs. the older multi-line layout.
