...
src/image_trainer/
  ...
  pipeline/
    ...
    resize.py          # scale shortest side → target_size, face-aware or center-crop, write NNNN.png
    face_detect.py     # MTCNN (facenet-pytorch) wrapper used by resize.py
...
| `trainer prep <project> [--source DIR] [--no-face-crop]` | Optional ingest from `--source` into `raw/`, then resize into `processed/` with face-aware rule-of-thirds crop (default) or center-crop (`--no-face-crop`). Face detection is via MTCNN (`facenet-pytorch`); if it's not installed or no face is found in a given image, that image falls back to center-crop and is marked `include=False` in `review.json`. `--no-face-crop` persists to `project.face_aware_crop` so subsequent runs honour it. |
...
2. **Prep** — `trainer prep` copies supported images (`.jpg/.jpeg/.png/.webp`) from `--source` into `raw/` (idempotent, `shutil.copy2`), then resizes to `target_size` square PNGs in `processed/` with zero-padded names (`0000.png`, `0001.png`, …). Two crop strategies:
   - **Face-aware (default).** `pipeline/face_detect.py` runs MTCNN over each source and picks the largest face. A rule-of-thirds intersection is chosen based on the face's natural quadrant in the source (upper-left face → upper-left intersection, etc.), then the image is scaled + cropped so the face centroid lands there, clamped to valid image bounds. Vertically-centred faces default to the upper third (portrait convention). Images where no face is detected are still written with a centre-crop fallback but their stems are marked `include=False` in `review.json` with a `prep: no face detected` note.
   - **Centre-crop fallback.** Classic behaviour — scale shortest side to `target_size` with LANCZOS, centre-crop. Used when `project.face_aware_crop=False`, `--no-face-crop` was passed, or `facenet-pytorch` isn't installed.
...
2. **Ingest (02)** — pick source folder + "Face-aware crop" checkbox (bound to `project.face_aware_crop`); dispatches `trainer prep --source <dir>` (adds `--no-face-crop` if the checkbox is unticked).
...
- **Face-aware crop needs `facenet-pytorch` installed manually with `--no-deps`.** The package pins `torch<2.3.0`, which would downgrade torch and break `bitsandbytes` / `diffusers` if installed normally. The correct install: `.venv/bin/python -m pip install facenet-pytorch --no-deps`. `pipeline/face_detect.py` soft-imports it and `resize.py` falls back to centre-crop when it's missing. Do NOT re-add a `[face]` extra to `pyproject.toml` — there's a comment in the file explaining why.
